...
function findAdults(arr) {
    let womanAdult = arr.filter(function(person) {
        if (person.age >= 18 && person.sex === 'female') { return person; }
    });

    let manAdult = arr.filter(function(person) {
        if (person.age >= 18 && person.sex === 'male') { return person; }
    });
    return [womanAdult, manAdult]
}
