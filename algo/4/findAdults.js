...
function findAdults(arr) {
  ...
  let hommesAdultes = [];
  let Adultes = [];
  persons.find((x) => x.sex === "female" && x.age >= 18);

  persons.find((y) => y.sex === "male" && y.age >= 18);
  femmesAdultes.push(x);
  hommes.Adultes.push(y);
}

findAdults(Adultes.push(femmesAdultes, hommesAdultes));
